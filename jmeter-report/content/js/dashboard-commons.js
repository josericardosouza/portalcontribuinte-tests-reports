...
/*
 * Sufixa o valor especificado com uma unidade
 * O argumento spaced define se um espaço será adicionado.
 */
...
/*
 * Obtém uma string representando a duração em milissegundos.
 * 
 * Ex.: duração = 20000100, retorna "45 min 20 seg 100 ms"
 */
function formatDuration(duration, spaced) {
    ...
    var days = Math.floor(duration / 86400000); // 1 dia
    duration %= 8640000;

    var hours = Math.floor(duration / 3600000); // 1 hora
    duration %= 3600000;

    var minutes = Math.floor(duration / 60000); // 1 minuto
    duration %= 60000;

    var seconds = Math.floor(duration / 1000); // 1 segundo
    duration %= 1000;
...
/*
 * Rótulo do eixo para a granularidade especificada
 */
...
function getConnectTimeLabel(granularity) {
    return "Tempo de conexão (granularidade: " + formatDuration(granularity) + ")";
}
...
function createLegend(choiceContainer, infos) {
    ...
    // Criar lista de séries com suporte de ativação/desativação
    ...
    // Recriar gráfico ao alternar séries
    choiceContainer.find("input").click(function(){
        infos.createGraph();
    });
}

// Desmarca todas as caixas – funcionalidade "Ocultar todas as amostras"
function uncheckAll(id){
    toggleAll(id, false);
}

// Marca todas as caixas – funcionalidade "Exibir todas as amostras"
